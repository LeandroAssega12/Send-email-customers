...
set ECHO off
SET COLSEP '|'
SPOOL liquidaciones_IRC.txt

SELECT 
  rl.report_log_path--,
 -- CONCAT('|', REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(df.doc_def_label,' ',''),'-',''),'(',''),')',''),':',''),'.','')) AS carpeta 
FROM im_trans it,
